...
{
    uint8_t pixel = uint8_t(gpio_get_all());
...
uint8_t ov7670::sendCommand_regs(const uint8_t** src)
{

    if (src == nullptr) {
        printf("Error: src is nullptr\n");
    ...
    while(1)
    {
        ...
        printf("0:%x, 1:%x",src[i][0],src[i][1]);
...
void ov7670::setFormat(resolution res)
{
    switch(res)
    {
        case resolution::VGA:
            ...
            sendCommand_regs((const uint8_t**)vga_ov7670);
        break;

        case resolution::QVGA:
            sendCommand(REG_COM3,4);//enable scaling
            sendCommand_regs((const uint8_t**)qvga_ov7670);
        break;

        case resolution::QQVGA:
            printf("start sending ");
            sendCommand(REG_COM3,4);//enable scaling
            printf("start sending regs");
            sendCommand_regs((const uint8_t**)qqvga_ov7670);
        break;

        case resolution::CIF:
            sendCommand(REG_COM3,4);//enable scaling
            sendCommand_regs((const uint8_t**)cif_ov7670);
        break;

        case resolution::QCIF:
            sendCommand(REG_COM3,4);//enable scaling
            sendCommand_regs((const uint8_t**)qcif_ov7670);
        break;
...
void ov7670::setImageType(IMG_Type Itype)
{
    ...
    switch(Itype)
    {
        case IMG_Type::RGB565:
            sendCommand_regs((const uint8_t**)rgb565_ov7670);
            freq=readReg(0x11);
            sleep_ms(1);
            sendCommand(0x11,freq);
        break;
        case IMG_Type::GRAY:
            sendCommand_regs((const uint8_t**)yuv422_ov7670);
        break;
        case IMG_Type::YUV:
            sendCommand_regs((const uint8_t**)yuv422_ov7670);
        break;
        case IMG_Type::RAW_RGB:
            sendCommand_regs((const uint8_t**)bayerRGB_ov7670);
        break;
